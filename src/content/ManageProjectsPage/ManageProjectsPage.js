...
class ManageProjectsPage extends Component {
  ...
  BatchAddProjects = async() => {
    ...
    let i=0;
    let domain = "";
    if (!document.getElementById("skipHeader").checked) {i=1}
    for (i; i<fileData.length; i++) {
      for (let id=0;id<this.state.domainList.length;id++){
        console.log(String(this.state.domainList[id].projectdomainname).toLowerCase())
        console.log(String(fileData[i][2]).toLowerCase())
        if (String(this.state.domainList[id].projectdomainname).toLowerCase() === String(fileData[i][2]).toLowerCase()) {
          domain = this.state.domainList[id].projectdomainid
        }
      }
      sequenceNumber++
      objProjectList.push({
      ...
      //resets the domain variable. The domain var will retain the value of the previous item.
      ...
      <>
            ...
            itemToElement={(item) => {
              return <>
                <div style={{display:'flex', justifyContent:'space-between'}}>
                  ...
                  <div style={{backgroundColor:item.projectdomaincolorhex}}>{item.projectdomaincolorhex}</div>
                </div>
              </>
            }}
        ...
        >
          ...
          <Tile className='file-import-warining-tile'>
            <div id="warningIcon"><WarningHex24/></div>
                    ...
                    <TableContainer title="Ideas" description={`Displays list of all ideas to be considered at the ISR. Total ideas: ${this.state.projectList.length}`}>
                      <TableToolbar>
                        ...
                        <Button
                          ...
                          />
                        <Button renderIcon={DocumentImport32} hasIconOnly iconDescription='Import Project List' kind='secondary' onClick={() => this.setState({modalImportOpen:true})}></Button>
                        <Button
                          kind='danger'
                          renderIcon={TrashCan32}
                          size='sm'
